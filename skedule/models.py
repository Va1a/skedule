...
class User(db.Model, UserMixin):
    ...
    shiftTemplates = db.relationship(
        "Template",
        secondary=user_shiftTemplate,
        backref="employees",
        cascade="all",
    )
    ...
    def external_id(self, value):
        self._external_id = str(value) if value is not None else None

    @external_id.expression
    def external_id(cls):
        return func.coalesce(cls._external_id, cast(cls.id, String))
...
class Shift(db.Model):
    ...
    def toJSON(self):
        assignments_json = (
            db.session.query(Assignment.id, Assignment.assignment_type)
            .filter_by(shift_id=self.id)
            .all()
        )
        return {
            "id": self.id,
            ...
            "duration": str(self.duration).zfill(4),
            "maxEmployees": self.maxEmployees,
            "minEmployees": self.minEmployees,
            "day_id": self.day_id,
            "assignments": [
                {"id": assignment.id, "type": assignment.assignment_type}
...
class Template(db.Model):
    ...
    def toJSON(self):
        return {
            ...
            "minEmployees": self.minEmployees,
            "duration": str(self.duration).zfill(4),
            "employees": [emp.external_id for emp in self.employees],
        }
...
class Alert(db.Model):
    ...
    timestamp = db.Column(db.DateTime, nullable=False, default=getLocalizedTime)
    seen = db.Column(db.Boolean, nullable=False, default=False)
    texted = db.Column(db.Boolean, nullable=False, default=False)
    emailed = db.Column(db.Boolean, nullable=False, default=False)

    recipient = db.relationship("User", backref="alerts")
