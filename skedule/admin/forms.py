...
TIME_VALIDATOR = Regexp(
    r"^([01]\d|2[0-3])([0-5]\d)$",
    message="Must be in 24-hour HHMM format!",
)
EMPLOYEE_LIST_VALIDATOR = Regexp(
    r"^[1-9]\d(?:, {0,1}[1-9]\d)*$",
    message="Input must be a comma seperated list of employee IDs.",
)
...
class AddShiftForm(FlaskForm):
    ...
    startTime = StringField("Start Time:", validators=[DataRequired(), TIME_VALIDATOR])
    duration = StringField("Duration:", validators=[DataRequired(), TIME_VALIDATOR])
    ...
    employees = StringField(
        "Assign Employees:",
        validators=[Optional(), Length(0, 512), EMPLOYEE_LIST_VALIDATOR],
    )
    ...
    def validate_maxEmployees(self, maxEmployees):
        if maxEmployees.data < self.minEmployees.data:
            raise ValidationError(
                "Maximum employee count cannot be less than the minimum!"
            )
