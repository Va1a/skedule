...
from cso.users.forms import LoginForm, RegisterForm
...
def register():
	...
	if form.validate_on_submit():
		...
		user = User(csoid=form.csoid.data, name=form.name.data, email=form.email.data, phone=form.phone.data, password=hashedpw)
		db.session.add(user)
		db.session.commit()
...
def login():
	...
	if form.validate_on_submit():
		...
		if user and bcrypt.check_password_hash(user.password, form.password.data):
			...
			flash(f'Logged in as CSO {user.csoid}.', 'success')
